...
async function loadOnThisDay(){
  ...
  var results=await Promise.allSettled(HISTORY_YEARS.map(function(yr){return ghFetch('history/'+yr+'.tsv');}));
  var matches=[];
...
function hatLoadingHtml(){return'<div class="hat-loading"><img class="hat-loading-img" src="https://dan2bit.github.io/live-shows/red_hat_icon.png" alt=""><div class="loading loading-dots" style="animation:none">Loading</div></div>';}
async function loadHistoryYear(yr){
  if(historyData[yr]!==null)return;
  try{
    var results=await Promise.all([ghFetch('history/'+yr+'.tsv'),new Promise(function(r){setTimeout(r,2400);})]);
    historyData[yr]=parseTsv(decodeURIComponent(escape(atob(results[0].content.replace(/\n/g,'')))));
  }catch(e){historyData[yr]=[];console.error('Failed to load history/'+yr+'.tsv:',e);}
}
...
// ── Please Tour Here ────────────────────────────
var fastTrackRows=[];
var FAST_TRACK_PATH='fast_track.tsv';
...
async function _loadSetlistsForYear(year){
  ...
  try{
    var fd=await ghFetch('setlists/'+year+'.json');
