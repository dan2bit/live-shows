...
|----------|---------|--------------|
| `private-data-guard.yml` | every push & PR | **The required `guard` check on `main`.** Fails any commit that introduces private sidecar paths (`live-shows-private/`, `*_private.tsv`, `*_caps.tsv`) or a TSV with a `Private Notes` header into this public repo. Layer-3 backstop for the 2026-06-27 leak (#76). |
...
| `potentials-maintenance.yml` | push to `main` touching potentials or current TSVs | Runs `reconcile_purchases.py` (#152: purchased shows reflected into potentials + fast_track + NAR) and `prune_potentials.py` (drops past-dated Pass rows), commits if changed; then warn-only checks: `check_brackets.py` (Prev/Next brackets) and `check_box_office.py` (#186 Box Office flag guardrail). Idempotent on the promote-back retrigger. |
...
| `audit-times-seen.yml` | push to `main` touching ledger sources | `scripts/audit_times_seen.py` — blocking check (#119) that artists.tsv "Times Seen" equals the canonical ledger count from the artist-index builder. |
| `reconcile-photos.yml` | push to `main` touching show TSVs or `artist-photos.tsv` | `scripts/reconcile_photos.py` (#131): every show Photo URL must match a Share Link row keyed on the Google Photos `/photo/<ID>`. CORRUPT (near-miss ID) fails; MISSING only reports. |
