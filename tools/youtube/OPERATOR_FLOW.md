...
`--apply` (with `--publish`) — plus an optional drop-folder front end for macOS
(`yt_drop.py`, below). Credentials and the surrounding utility scripts are
...
## The drop-folder flow  *(macOS, optional)*

The same stages, driven from Finder. `yt_drop.py` (next to this file) watches a
folder, runs the scan for you, and leaves double-clickable buttons beside the
clips for the steps that need you in between. It calls the stages below exactly
as documented; nothing in it talks to YouTube on its own.

```
~/Bootlegs/inbox/                          drop the zip(s) here
~/Bootlegs/2026-09-19-lake-street-dive/    renamed from the show the scan resolved
    PXL_*.mp4 ...
    SCAN.txt                               step 2's output - read it over coffee
    README.txt
    1 - Upload.command                     step 4 (lint, dry run, confirm, upload)
    2 - Edit setlist.command               step 6 (identify, then the editor)
    3 - Publish everything.command         step 6 apply --publish, then step 7,
                                           prints the playlist URL, then cleans up
    Open manifest.command
    Rescan with a date.command             only if the scan could not resolve the show
```

One-time setup, from `tools/youtube` with the venv active:

```bash
python3 yt_drop.py install        # writes and loads a LaunchAgent watching ~/Bootlegs/inbox
python3 yt_drop.py status         # what is in flight; uninstall to remove the agent
```

Then a night is: download the zip from Google Photos into the inbox (a split
export's `Photos-001.zip`, `-002.zip` all go in; they are extracted into one
folder), wait for the notification, read `SCAN.txt` and fix the manifest, press
1, do the Studio pass after the upload, press 2, press 3. Rotate sideways clips
before pressing 1.

Every button passes its own folder as `--clips`, and the scan records the folder
in the manifest's `.scan.json`, so two shows can be in flight at once. After a
...
If the scan cannot resolve the show (no row for that date, or clips from two
nights in one zip) the folder keeps a timestamp name, `SCAN-FAILED.txt` holds the
reason, and the only button is the rescan one, which asks for a date.
