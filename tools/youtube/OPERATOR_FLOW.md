...
`close-playlist-issue.yml` writes it to the show row. The inventory TSVs
(`youtube_videos.tsv`, `youtube_playlists.tsv`, the correlation file) are
regenerated by `youtube-inventory.yml` on a schedule and after a dispatch, so
the local copies `youtube_fetch.py` rewrites are just that - local.
