...
3 - Publish everything.command titles, publishes, builds the playlist, prints its
                               ...
                               folder. Paste the URL as a COMMENT on the playlist
                               issue: CI writes it to the show row and closes.
...
echo
# Terminal keeps a .command window open after the shell exits unless the
# profile says otherwise; ask it to close this one by its tty instead.
...
def cmd_publish(args):
    ...
    if rc != 0:
        print("\nFetch failed; the playlist step needs it. Press again to retry from here "
              "(apply is idempotent).")
        return rc
    # No --update-history: the show-row write-back is done by CI from the
    # playlist issue (close-playlist-issue.yml reacts to a comment carrying
    # the URL), so nothing here leaves the local clone with an uncommitted
    # data/ change. The inventory TSVs youtube_fetch.py just rewrote are
    # regenerated by CI too; they need no commit from this machine.
    print("\n== youtube_create_playlists.py --new-show")
    rc, out = run([python(), PLAYLISTS, "--new-show", date], capture=True)
    if rc != 0:
        return rc
    ...
    if urls:
        ...
        print("Paste that URL as a comment on the playlist issue. CI writes it to the show "
              "row and closes the issue; nothing to commit here.")
        if sys.platform == "darwin":
            subprocess.run(["pbcopy"], input=urls[-1], text=True, check=False)
            print("(copied to the clipboard)")
    else:
        print("Playlist step finished but printed no playlist URL - check the output above.")
    print("=" * 72)
    if shutil.which("gh"):
        r = subprocess.run(["gh", "workflow", "run", "youtube-inventory.yml"],
                           cwd=REPO_ROOT, capture_output=True, text=True)
        print("inventory refresh dispatched to CI" if r.returncode == 0
              else f"could not dispatch youtube-inventory.yml ({r.stderr.strip()}); "
                   "it runs nightly anyway")
